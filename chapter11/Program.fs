            ...
            if Array.length variant <> Array.length branch then
                ...
                let check state branch =
                    ...
                    | Some(_, type_) ->
                        ...
                        | Some s, t when equal ctx s t -> Some t
                        | Some _, _ -> raise (TypeError "case branch type mismatch")
                        | None, t -> Some t
                    | None -> raise (TypeError "unknown tag found in case")

                match Array.fold check None branch with
                | Some t -> t
                | None -> raise (TypeError "case branch cannot be empty")
...
let rec eval ctx term =
    ...
    | Bin { left = left; op = op; right = right } ->
        ...
        | a ->
            printfn "%s" (string a)
            raise (RuntimeError "invalid type for binary experssion")
...
let rec to_string res =
    ...
    | RRecord r ->
        ...
        + "}"
    | RAbs _ -> "FUNCTION"
    | RTag t -> $"{t.tag}({to_string t.value})"
